/**
 * Copyright 2023 Ashu. All rights reserved.
 * Use of this source code is governed by a MIT-style license
 */

export const data = [
  {
    image: require('./assets/1.png'),
    name: 'Ashu',
    message: 'Hey, how do I implement this scroll effect in react-native?',
  },
  {
    image: require('./assets/2.png'),
    name: 'Dave Knox',
    message: 'You loved "I\'ll try to let them down easy"',
  },
  {
    image: require('./assets/3.png'),
    name: 'Allison Cain',
    message: 'Attachment: 1 Video',
  },
  {
    image: require('./assets/4.png'),
    name: 'Shelley Willems',
    message: '👍🏼🎉🍰',
  },
  {
    image: require('./assets/5.png'),
    name: 'Beam Seilaudom',
    message: 'Us too!',
  },
  {
    image: require('./assets/6.png'),
    name: 'Paige Hall',
    message: 'Yeah, I just got the group link - thanks for adding me 😎',
  },
  {
    image: require('./assets/7.png'),
    name: 'Maya Johnson',
    message:
      "Ok, I'm Picking everyone up at 8am tomorrow, Be ready to pile into the minivan my friend!",
  },
  {
    image: require('./assets/8.png'),
    name: 'Michael Kirk',
    message: 'This brownie recipe is good beyond words',
  },
  {
    image: require('./assets/9.png'),
    name: 'Isabelle van Rijn',
    message: 'Best breakfast ever',
  },
  {
    image: require('./assets/1.png'),
    name: 'Alicia García',
    message: "It was great to see you! Let's catch up again soon",
  },
  {
    image: require('./assets/2.png'),
    name: 'Roberto',
    message: 'Say hello to Alice',
  },
  {
    image: require('./assets/3.png'),
    name: 'Alicia Torreaux',
    message: 'Bob says hi',
  },
  {
    image: require('./assets/4.png'),
    name: 'Hanna Schmidt',
    message: "Sorry, I'll be late. 😬 about 15 minutes",
  },
  {
    image: require('./assets/5.png'),
    name: 'Emily Yeung',
    message: 'For sure! Consistency is key.',
  },
  {
    image: require('./assets/6.png'),
    name: 'Drew Bean',
    message: 'Replied to your story: 💯',
  },
  {
    image: require('./assets/7.png'),
    name: 'Lisa Goldman',
    message: 'At this point, I am not suprised',
  },
  {
    image: require('./assets/8.png'),
    name: 'Maya Ortiz',
    message: 'Wanna get food?',
  },
  {
    image: require('./assets/9.png'),
    name: 'Margot Schröder',
    message: 'You sent a voice message',
  },
  {
    image: require('./assets/1.png'),
    name: 'Paris Kirby',
    message: 'I am running out of ideas..',
  },
  {
    image: require('./assets/2.png'),
    name: 'Ricky Padilla',
    message: 'Liked a message',
  },
];
